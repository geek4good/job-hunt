// src/scrapers/dice.ts — Dice.com tech jobs scraper
//
// Client-side SPA — requires Lightpanda (set LPD_TOKEN or LPD_CDP_URL).
...
const URL = "https://www.dice.com/jobs?q=software+engineer&remote=true&limit=50";
...
function parseJobsFromHtml(html: string): Job[] {
  ...
  for (const item of extractJsonLd(html)) {
    ...
    jobs.push({
      ...
      source: "dice",
      region: String((d.jobLocation as Record<string, unknown>)?.name ?? ""),
      category: "",
    });
  }
  return jobs;
